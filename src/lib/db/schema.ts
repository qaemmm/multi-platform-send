...
export const publishPresets = sqliteTable('publish_presets', {
  ...
  // 发布设置
  isOriginal: integer('is_original', { mode: 'boolean' }).default(true), // 是否原创
  enableReward: integer('enable_reward', { mode: 'boolean' }).default(false), // 是否开启赞赏
  autoSelectCover: integer('auto_select_cover', { mode: 'boolean' }).default(true), // 自动选择封面
  autoGenerateDigest: integer('auto_generate_digest', { mode: 'boolean' }).default(true), // 自动生成摘要

  // 内容增强设置
  enableFeaturedArticles: integer('enable_featured_articles', { mode: 'boolean' }).default(false), // 是否启用精选文章（智能匹配）
  customContent: text('custom_content'), // 末尾定制富文本内容
  recommendedAccount: text('recommended_account'), // 推荐的公众号名称
